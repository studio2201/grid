# =============================================================================
# grid — Alpine container image (Unraid-compatible)
# =============================================================================
...
#     GRID_DATA_DIR env var if needed.
#   - Listens on 0.0.0.0:<port> for bridge networking.
#
# Build (Podman):
#   cd ~/projects/studio2201/grid
#   podman build -f Containerfile.ubi -t grid:latest .
#
# Run (Unraid-style):
#   podman run -d --name grid \
#     -p <host-port>:4405 \
#     -v /mnt/user/appdata/grid:/config \
#     -e GRID_PIN=<your-pin> \
#     grid:latest
#

# Compare size:
#   podman images | grep -E 'grid|studio2201/grid'
# =============================================================================
...
LABEL org.opencontainers.image.title="grid" \
      org.opencontainers.image.description="grid (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/grid" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/grid
RUN chmod +x /usr/local/bin/grid
...
ENV PORT=4405 \
    RUST_LOG=info \
    GRID_DATA_DIR=/config


EXPOSE 4405

HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4405}/health" >/dev/null || exit 1
